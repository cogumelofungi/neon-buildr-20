// Force redeploy - v2.18.0 - Support nullable app_id with LEFT JOIN
...
serve(async (req) => {
  ...
  try {
    ...
    if (provider === "Perfect Pay") {
      const ppStatus = String(body.sale_status_enum_key || body.sale_status_detail || body.sale_status_enum || body.status || "").toLowerCase();
    ...
    if (!isOrderBumpOnly) {
      ...
      // 🔐 FALLBACK: Se Stripe e produto não encontrado, tentar buscar stripe_api_key
      if (!product && provider === 'Stripe') {
        console.log('🔍 [STRIPE] Produto não encontrado. Buscando stripe_api_key...');
        
        // Buscar em products E order_bumps
        ...
        // Se não encontrou em products, buscar em order_bumps
        let stripeApiKey = stripeProducts?.stripe_api_key;
        if (!stripeApiKey) {
          console.log('🔍 [STRIPE] Buscando stripe_api_key em order_bumps...');
          const { data: stripeOrderBump } = await supabase
            .from('order_bumps')
            .select('stripe_api_key, product_id')
            .eq('provider', 'Stripe')
            .not('stripe_api_key', 'is', null)
          ...
          stripeApiKey = stripeOrderBump?.stripe_api_key;
          if (stripeApiKey) {
            console.log('✅ [STRIPE] stripe_api_key encontrada em order_bumps');
          }
        }
          ...
          try {
            ...
            if (fullSession.line_items?.data?.[0]?.price?.product) {
              ...
              if (realProduct) {
                ...
                // Tentar encontrar como order bump
                console.log('🔍 [STRIPE] Produto não encontrado em products, verificando order_bumps...');
                const { data: obProduct } = await supabase
                  .from('order_bumps')
                  .select('id, app_id, label, app_link, default_language, hottok, postback_key, webhook_token, stripe_api_key, api_token, client_id, client_secret, basic_token, account_id, store_slug, yampi_secret_key')
                  .eq('product_id', realProductId)
                  .eq('provider', 'Stripe')
                  .eq('is_active', true)
                  .maybeSingle();
                ...
                if (obProduct) {
                  console.log('✅ [STRIPE] Order bump encontrado após busca via API');
                  isOrderBumpOnly = true;
                  orderBumpWithTokens = obProduct;
                  // Criar produto virtual
                  product = {
                    id: `ob_${obProduct.id}`,
                    app_id: obProduct.app_id,
                    app_link: obProduct.app_link,
                    hottok: obProduct.hottok,
                    postback_key: obProduct.postback_key,
                    webhook_token: obProduct.webhook_token,
                    stripe_api_key: obProduct.stripe_api_key,
                    cartpanda_bearer_token: obProduct.api_token,
                    cartpanda_store_slug: obProduct.store_slug,
                    default_language: obProduct.default_language || 'pt-br',
                    apps: null,
                    _isOrderBumpVirtual: true,
                    _orderBumpId: obProduct.id,
                    _orderBumpLabel: obProduct.label,
                  };
                }
              }
            }
    ...
    if (isOrderBumpOnly && orderBumpWithTokens) {
      ...
      product = {
        ...
        postback_key: orderBumpWithTokens.yampi_secret_key || orderBumpWithTokens.postback_key,
    ...
    // VALIDAÇÃO STRIPE - Webhook Signing Secret (opcional - permite proxy sem signature)
    if (provider === "Stripe") {
      const stripeSignature = req.headers.get('stripe-signature');
      const proxyHeader = req.headers.get('x-proxy-by');
      
      if (!stripeSignature) {
        if (proxyHeader) {
          console.log('ℹ️ [STRIPE] Stripe-Signature ausente mas veio via proxy - continuando sem validação de signature');
        } else {
          console.warn('⚠️ [STRIPE] Stripe-Signature header ausente - continuando sem validação');
        }
      } else if (product.webhook_token) {
        // Se temos tanto a signature quanto o webhook_token, validar
        try {
          const Stripe = (await import('https://esm.sh/stripe@18.5.0')).default;
          const stripeClient = new Stripe(product.stripe_api_key || 'sk_dummy', { apiVersion: '2023-10-16' });
          const cryptoProvider = Stripe.createSubtleCryptoProvider();
          
          await stripeClient.webhooks.constructEventAsync(
            rawBody,
            stripeSignature,
            product.webhook_token,
            undefined,
            cryptoProvider
          );
          console.log('✅ [STRIPE] Webhook Signature validada com sucesso');
        } catch (sigError) {
          console.warn('⚠️ [STRIPE] Validação de signature falhou (pode ser proxy):', sigError instanceof Error ? sigError.message : String(sigError));
          // Não bloquear - permitir processamento mesmo com signature inválida via proxy
        }
      } else {
        console.log('ℹ️ [STRIPE] Webhook Signing Secret não configurado no produto - pulando validação');
      }
    }
...
async function normalizeWebhookData(provider: string, body: any) {
  switch (provider) {
    ...
    case "Mundpay": {
      ...
      // Product ID: priorizar product.id (real) sobre offer.id
      const productId = firstOffer.product?.id || firstOffer.id || firstOffer.sku || "unknown";
