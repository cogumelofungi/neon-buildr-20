...
export const InternalPdfViewer = ({
  ...
  // Mozilla PDF.js Viewer (versão estável fixada via CDNJS)
  const PDFJS_VIEWER = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.0.379/web/viewer.html';
  const getPdfViewerUrl = () => {
    // iOS: use direct URL (fallback original preserved), non-iOS: proxy for CORS
    const finalUrl = isIOS ? pdfUrl : (isStorageUrl(pdfUrl) ? pdfProxyUrl(pdfUrl) : pdfUrl);
    console.log('[InternalPdfViewer] targetUrl para PDF.js:', finalUrl);
    return `${PDFJS_VIEWER}?file=${encodeURIComponent(finalUrl)}&locale=pt-BR`;
  };
