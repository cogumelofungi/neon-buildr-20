...
import { subscribeToPlan } from "@/api/planService";
...
const CreditCardForm = ({ planName, amount, planId, billing = 'monthly', monthlyPrice, onPaymentSuccess, onCancel }: CreditCardFormProps) => {
  ...
  const onSubmit = async (data: CreditCardFormData) => {
    ...
    try {
      // Structure payment data for plan service
      const paymentData = {
        paymentMethod: {
          type: 'card' as const,
          card: {
          ...
          billingDetails: {
            ...
            phone: data.phone,
            address: {
              line1: `${data.address}, ${data.number}`,
      ...
      const result = await subscribeToPlan(planId, user.id, paymentData);
      ...
      toast({
        ...
        description: `Sua assinatura do plano ${planName} foi ativada com sucesso.`,
      });
