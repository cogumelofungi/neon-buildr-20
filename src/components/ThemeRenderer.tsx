...
const ThemeRenderer = ({
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end text-center pb-8 px-5">
            {/* App Icon */}
            {appData.showAppIcon !== false && appData.icone_url && (
              <div
            ...
            {appData.showAppIcon !== false && isPreview && !appData.icone_url && (
              <div
                className="w-14 h-14 rounded-full overflow-hidden mb-3 border-2 border-white/30 shadow-lg flex items-center justify-center"
                style={{ backgroundColor: appData.cor }}
              >
                <ImageIcon className="w-6 h-6" style={{ color: "#A3C1FB", opacity: 0.5 }} />
              </div>
              ...
              >
                ...
                {appData.allow_pdf_download && product.url && (
                  <div
                    ...
                    onClick={(e) => { e.stopPropagation(); handleDownload(product.url!, `${product.label || 'file'}.pdf`); }}
                  >
          ...
          <div className="overflow-auto">
            <div className="grid grid-cols-2 gap-3 pb-4">
              ...
              {appData.videoCourseEnabled && (
                <button
                  onClick={() => setIsModulesDialogOpen(true)}
                  className={`relative aspect-[3/4] rounded-2xl overflow-hidden outline-none focus:outline-none ${appData.flowShowCardBorder ? 'border-2' : ''}`}
                  style={{
  ...
  const renderAcademyTemplate = () => {
    ...
    // No preview: mostra só os que têm imagem, mas se nenhum tiver, mostra todos como placeholder
    // No app publicado: mostra apenas os que têm imagem
    const slotsWithImage = allCarouselSlots.filter(product => product.thumbnail);
    const featuredProducts = isPreview 
      ? (slotsWithImage.length > 0 ? slotsWithImage : allCarouselSlots)
      : slotsWithImage;
    ...
    return (
      <div className="h-full w-full overflow-auto" style={{ backgroundColor: isDark ? "#000000" : "#ffffff" }}>
        ...
        {featuredProducts.length === 0 && (appData.showAppIcon !== false) && (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {allProducts.map((product, index) => (
              ...
              >
                ...
                {/* Download button */}
                {appData.allow_pdf_download && product.url && !isPreview && (
                  <div
                    className="absolute top-2 right-2 z-10 w-7 h-7 rounded-full bg-black/40 flex items-center justify-center cursor-pointer hover:bg-black/60 transition-colors"
                    onClick={(e) => { e.stopPropagation(); handleDownload(product.url!, product.label || 'download'); }}
                  >
                    <Download className="w-3.5 h-3.5 text-white" />
                  </div>
                )}
