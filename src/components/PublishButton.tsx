...
const PublishButton = ({ appBuilder }: PublishButtonProps) => {
  ...
  useEffect(() => {
    const checkRepublication = async () => {
      // Se temos publishedSlug, é republicação
      if (appBuilder.appData.publishedSlug) {
        setIsRepublishing(true);
        return;
      }

      // Se não tem customLink, não é republicação
    ...
    checkRepublication();
  }, [appBuilder.appData.customLink, appBuilder.appData.publishedSlug]);

  const handlePublishClick = async () => {
    // Verificar limite considerando se é atualização
    const slugToCheck = appBuilder.appData.publishedSlug || appBuilder.appData.customLink.trim();
    await refreshLimit(slugToCheck);
