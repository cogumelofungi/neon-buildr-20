...
export const PremiumOverlay = ({
  ...
  // Função para redirecionar para pricing
    ...
    return (
      <div className={cn("relative", className)}>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/10 rounded-lg">
          <button
            onClick={handleUpgradeClick}
            className="bg-black/80 rounded-lg px-3 py-2 text-center border border-orange-500/30
                       hover:bg-black/90 hover:border-orange-500/50 
                       transition-all duration-200 cursor-pointer"
          >
            <Crown className="w-5 h-5 text-orange-400 mx-auto mb-1" />
            <p className="text-white text-xs font-medium">{t(requiredPlan)}</p>
          </button>
        </div>
      </div>
    );
    ...
    <div className={cn("relative", className)}>
      ...
      <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] rounded-lg z-20 flex items-center justify-center">
        <div className="bg-black/80 rounded-xl p-4 mx-4 text-center border border-orange-500/30">
          ...
          {description && <p className="text-white/70 text-xs mb-3">{description}</p>}
          <button
            onClick={handleUpgradeClick}
            className="bg-orange-500/20 text-orange-300 px-3 py-1 rounded-full text-xs font-medium 
                       hover:bg-orange-500/30 hover:text-orange-200 
