...
export const useAuthActions = () => {
  ...
  const signUp = async ({ email, password, fullName, phone }: SignUpData) => {
    try {
      ...
      let preferredLanguage = 'pt-br'; // padrão
      ...
      // Chamar edge function para adicionar ao Brevo (cadastro gratuito)
      try {
        await supabase.functions.invoke('brevo-new-registration', {
          body: { email, fullName }
        });
      } catch (brevoErr) {
        console.log('Brevo integration (non-critical):', brevoErr);
      }
