...
const CACHE_NAME = 'published-app-pwa-v4.2';
const STATIC_CACHE = 'published-app-static-v2';
...
console.log('[SW-APP v4.2] Service Worker iniciado');

self.addEventListener('install', (event) => {
  console.log('[SW-APP] Instalando v4.2...');
  self.skipWaiting();
});

self.addEventListener('activate', (event) => {
  console.log('[SW-APP] Ativando v4.2...');
  event.waitUntil(
    caches.keys().then((keys) =>
      ...
      console.log('[SW-APP] Service Worker v4.2 ativado e pronto');
...
self.addEventListener('fetch', (event) => {
  ...
  // Network-first para recursos estáticos (garante atualizações imediatas)
  event.respondWith(
    fetch(req)
      .then((res) => {
        if (res && res.status === 200 && res.type === 'basic') {
      ...
      .catch(() => {
        return caches.match(req).then((cached) => {
          if (cached) {
            console.log('[SW-APP] Offline fallback:', url.pathname);
            return cached;
          }
...
console.log('[SW-APP] Service Worker v4.2 carregado com sucesso');
